...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Enlaces del footer para filtrar por tipo
    document.getElementById('link-gatos')?.addEventListener('click', e => {
        e.preventDefault();
        localStorage.setItem("seleccion", JSON.stringify({ tipo: "gato", edad: "" }));
        window.location.href = "gatosoperros.html";
    });
    document.getElementById('link-perros')?.addEventListener('click', e => {
        e.preventDefault();
        localStorage.setItem("seleccion", JSON.stringify({ tipo: "perro", edad: "" }));
        window.location.href = "gatosoperros.html";
    });

    // Enlaces del footer para mostrar secciones de adopción o apadrinamiento
    document.getElementById('link-adopcion')?.addEventListener('click', e => {
        e.preventDefault();
        mostrarCuerpoWebAdopcion();
        window.scrollTo({ top: 0, behavior: 'smooth' });
    });
    document.getElementById('link-apadrinamiento')?.addEventListener('click', e => {
        e.preventDefault();
        mostrarCuerpoWebApadrinar();
        window.scrollTo({ top: 0, behavior: 'smooth' });
    });
});
